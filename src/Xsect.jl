# Xsect.jl
#
# Cross section geometry functions, adapted from EPA SWMM
#
...
const RECT_ALFMAX        = 0.97
const RECT_TRIANG_ALFMAX = 0.98
const RECT_ROUND_ALFMAX  = 0.98

const PI = 3.141592654

# This is included in the main SWMMBackwardsInference module
# include("./Xsectdat.jl")    # File containing geometry tables for rounded shapes

################################################################################
# Constants
################################################################################

@enum XsectType begin
      DUMMY                           # 0      
      CIRCULAR                        # 1      closed
...
@kwdef mutable struct TXsect
       xsecttype::XsectType=NULL            # type code of cross section shape
       culvertCode::Int=-1     # type of culvert (if any)
       transect::Int=-1        # index of transect/shape (if applicable)
       yFull::Real=-1       # depth when full (ft)
       wMax::Real=-1        # width at widest point (ft)
       ywMax::Real=-1       # depth at widest point (ft)
       aFull::Real=-1      # area when full (ft2)
       rFull::Real=-1      # hyd. radius when full (ft)
       sFull::Real=-1       # section factor when full (ft^4/3)
       sMax::Real=-1     # section factor at max. flow (ft^4/3)
    
       # These variables have different meanings depending on section shape
       yBot::Real=-1   # depth of bottom section
       aBot::Real=-1   # area of bottom section
       sBot::Real=-1   # slope of bottom section
       rBot::Real=-1   # radius of bottom section
end

struct TXsectStar
    s::Real              # section factor
    qc::Real             # critical flow
    xsect::TXsect        # pointer to a cross section object
end
...
function xsect_stringTypeToCode(xsecttype::String)
#  Input: type = type of xsection shape, as a string
#  Output: returns the int code for the string
    if     xsecttype == "DUMMY";             return DUMMY      
    elseif xsecttype == "CIRCULAR";          return CIRCULAR          #closed
    elseif xsecttype == "FILLED_CIRCULAR";   return FILLED_CIRCULAR   #closed
    elseif xsecttype == "RECT_CLOSED";       return RECT_CLOSED       #closed
    elseif xsecttype == "RECT_OPEN";         return RECT_OPEN       
    elseif xsecttype == "TRAPEZOIDAL";       return TRAPEZOIDAL       
    elseif xsecttype == "TRIANGULAR";        return TRIANGULAR       
    elseif xsecttype == "PARABOLIC";         return PARABOLIC
    elseif xsecttype == "POWERFUNC";         return POWERFUNC     
    elseif xsecttype == "RECT_TRIANGULAR";   return RECT_TRIANG # changed name
    elseif xsecttype == "RECT_ROUND";        return RECT_ROUND
    elseif xsecttype == "MOD_BASKET";        return MOD_BASKET
    elseif xsecttype == "HORIZ_ELLIPSE";     return HORIZ_ELLIPSE     #closed
    elseif xsecttype == "VERT_ELLIPSE";      return VERT_ELLIPSE      #closed
    elseif xsecttype == "ARCH";              return ARCH              #closed
    elseif xsecttype == "EGGSHAPED";         return EGGSHAPED         #closed
    elseif xsecttype == "HORSESHOE";         return HORSESHOE         #closed
    elseif xsecttype == "GOTHIC";            return GOTHIC            #closed
    elseif xsecttype == "CATENARY";          return CATENARY          #closed
    elseif xsecttype == "SEMIELLIPTICAL";    return SEMIELLIPTICAL    #closed
    elseif xsecttype == "BASKETHANDLE";      return BASKETHANDLE      #closed
    elseif xsecttype == "SEMICIRCULAR";      return SEMICIRCULAR      #closed
    elseif xsecttype == "IRREGULAR";         return IRREGULAR
    elseif xsecttype == "CUSTOM";            return CUSTOM            #closed
    elseif xsecttype == "FORCE_MAIN";        return FORCE_MAIN        #closed
    elseif xsecttype == "STREET_XSECT";      return STREET_XSECT
...
function xsect_isOpen(xsecttype::XsectType)
#  Input:   xsecttype = type of xsection shape
#  Output:  returns 1 if xsection is open, 0 if not
#  Purpose: determines if a xsection type is open or closed.
# TODO: fix
    return ((Amax[xsecttype] >= 1.0) ? 1 : 0)
end

################################################################################

function xsect_setParams(xsect::TXsect, xsecttype::XsectType, p::Vector{Float64}, ucf::Float64)
#  Input:   xsect = ptr. to a cross section data structure
#           xsecttype = xsection shape xsecttype
...
#
    ...
    if xsecttype != DUMMY && p[1] <= 0.0 return false end
    xsect.xsecttype  = xsecttype;
    if xsect.xsecttype == DUMMY
        xsect.yFull = TINY;
        xsect.wMax  = TINY;
        xsect.aFull = TINY;
        xsect.rFull = TINY;
    ...
    elseif xsect.xsecttype == FILLED_CIRCULAR
        if p[2] >= p[1]; return false end
    ...
    elseif xsect.xsecttype == CATENARY
        xsect.yFull = p[1]/ucf;
    ...
    elseif xsect.xsecttype == SEMIELLIPTICAL
        xsect.yFull = p[1]/ucf;
    ...
    elseif xsect.xsecttype == BASKETHANDLE
        xsect.yFull = p[1]/ucf;
    ...
    elseif xsect.xsecttype == SEMICIRCULAR
        xsect.yFull = p[1]/ucf;
    ...
    elseif xsect.xsecttype == RECT_CLOSED
        if p[2] <= 0.0; return false end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
        ...
        xsect.sMax = aMax * rect_closed_getRofA(xsect, aMax) ^ (2.0/3.);
        xsect.ywMax = xsect.yFull;

    elseif xsect.xsecttype == RECT_OPEN
        if p[2] <= 0.0; return false end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
        if p[3] < 0.0 || p[3] > 2.0; return false end   # sides to ignore
        xsect.sBot = p[3];
        xsect.aFull = xsect.yFull * xsect.wMax;
    ...
    elseif xsect.xsecttype == RECT_TRIANG
        if p[2] <= 0.0 || p[3] <= 0.0; return false end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
        xsect.yBot  = p[3]/ucf;
        xsect.ywMax = xsect.yFull;
        ...
        xsect.sMax  = aMax * rect_triang_getRofA(xsect, aMax) ^ (2.0/3.);

    elseif xsect.xsecttype == RECT_ROUND
        if p[2] <= 0.0; return false end
        if p[3] < p[2]/2.0; p[3] = p[2]/2.0 end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
        xsect.rBot  = p[3]/ucf;
        ...
        xsect.sMax = aMax * rect_round_getRofA(xsect, aMax) ^ (2.0/3.);

    elseif xsect.xsecttype == MOD_BASKET
        if p[2] <= 0.0; return false end
        if p[3] < p[2]/2.0; p[3] = p[2]/2.0 end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
    ...
    elseif xsect.xsecttype == TRAPEZOIDAL
        if p[2] < 0.0 || p[3] < 0.0 || p[4] < 0.0; return false end
        xsect.yFull = p[1]/ucf;
        xsect.ywMax = xsect.yFull;
        ...
        if xsect.yBot == 0.0 && xsect.sBot == 0.0; return false end
    ...
    elseif xsect.xsecttype == TRIANGULAR
        if p[2] <= 0.0; return false end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
        xsect.ywMax = xsect.yFull;
    ...
    elseif xsect.xsecttype == PARABOLIC
        if p[2] <= 0.0; return false end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
        xsect.ywMax = xsect.yFull;
        ...
        xsect.aFull = (2.0/3.) * xsect.yFull * xsect.wMax;
        xsect.rFull = xsect_getRofY(xsect, xsect.yFull);
        xsect.sFull = xsect.aFull * xsect.rFull ^ (2.0/3.);
        xsect.sMax  = xsect.sFull;

    elseif xsect.xsecttype == POWERFUNC
        if p[2] <= 0.0 || p[3] <= 0.0; return false end
        xsect.yFull = p[1]/ucf;
        xsect.wMax  = p[2]/ucf;
        xsect.ywMax = xsect.yFull;
        ...
        xsect.rBot  = xsect.wMax / (xsect.sBot + 1) /
                       xsect.yFull ^ xsect.sBot;
        xsect.aFull = xsect.yFull * xsect.wMax / (xsect.sBot+1);
        xsect.rFull = xsect_getRofY(xsect, xsect.yFull);
        xsect.sFull = xsect.aFull * xsect.rFull ^ (2.0/3.);
        xsect.sMax  = xsect.sFull;

    elseif xsect.xsecttype == HORIZ_ELLIPSE
        if p[2] == 0.0; p[3] = p[1] end
        if p[3] > 0.0                        # std. ellipse pipe
            index = (int)floor(p[3]) - 1;        # size code
            if ( index < 0 || index >= NumCodesEllipse ) return false end
    ...
    elseif xsect.xsecttype == VERT_ELLIPSE
        if p[2] == 0.0; p[3] = p[1] end
        if p[3] > 0.0                        # std. ellipse pipe
            index = (int)floor(p[3]) - 1;        # size code
            if ( index < 0 || index >= NumCodesEllipse ) return false end
    ...
    elseif xsect.xsecttype == ARCH
        if p[2] == 0.0; p[3] = p[1] end
        if p[3] > 0.0                        # std. arch pipe
            index = (int)floor(p[3]) - 1;        # size code
            if index < 0 || index >= NumCodesArch; return false end
...
#     xsect.sFull = xsect.aFull * xsect.rFull ^ (2.0/3.);
#     xsect.sMax  = Shape[index].sMax * yFull * yFull * yFull ^ (2.0/3.);
...
function xsect_getAmax(xsect::TXsect)
#  Input:   xsect = ptr. to a cross section data structure
#  Output:  returns area (ft2)
#  Purpose: finds xsection area at maximum flow depth.
    if xsect.xsecttype == IRREGULAR return xsect.aBot
    elseif xsect.xsecttype == CUSTOM return xsect.aBot
    else return Amax[xsect.xsecttype] * xsect.aFull;
    end
end

################################################################################

function xsect_getSofA(xsect::TXsect, a::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           a = area (ft2)
...
#  Purpose: computes xsection's section factor at a given area.
    alpha = a / xsect.aFull
    # double r;
      if xsect.xsecttype == FORCE_MAIN || xsect.xsecttype == CIRCULAR
      ...
      elseif xsect.xsecttype == RECT_ROUND
        ...
        if a == 0.0; return 0.0 end
        r = xsect_getRofA(xsect, a);
        if r < TINY; return 0.0 end
        return a * r ^ (2.0/3.);
      end
end
...
function xsect_getYofA(xsect::TXsect, a::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           a = area (ft2)
...
#  Purpose: computes xsection's depth at a given area.
    alpha = a / xsect.aFull;
      ...
      elseif xsect.xsecttype == RECT_CLOSED return a / xsect.wMax;

      elseif xsect.xsecttype == RECT_TRIANG return rect_triang_getYofA(xsect, a);

      elseif xsect.xsecttype == RECT_ROUND  return rect_round_getYofA(xsect, a);

      elseif xsect.xsecttype == RECT_OPEN   return a / xsect.wMax;

      elseif xsect.xsecttype == MOD_BASKET  return mod_basket_getYofA(xsect, a);

      elseif xsect.xsecttype == TRAPEZOIDAL return trapez_getYofA(xsect, a);

      elseif xsect.xsecttype == TRIANGULAR  return triang_getYofA(xsect, a);

      elseif xsect.xsecttype == PARABOLIC   return parab_getYofA(xsect, a);

      elseif xsect.xsecttype == POWERFUNC   return powerfunc_getYofA(xsect, a);
...
function xsect_getAofY(xsect::TXsect, y::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           y = depth (ft)
#  Output:  returns area (ft2)
#  Purpose: computes xsection's area at a given depth.
    yNorm = y / xsect.yFull;
    if y <= 0.0; return 0.0 end
      if xsect.xsecttype == FORCE_MAIN || xsect.xsecttype == CIRCULAR
        # println("  yNorm = $(y) / $(xsect.yFull)")
     ...
     elseif xsect.xsecttype == RECT_CLOSED  return y * xsect.wMax;

      elseif xsect.xsecttype == RECT_TRIANG return rect_triang_getAofY(xsect, y);

      elseif xsect.xsecttype == RECT_ROUND  return rect_round_getAofY(xsect, y);

      elseif xsect.xsecttype == RECT_OPEN   return y * xsect.wMax;

      elseif xsect.xsecttype == MOD_BASKET  return mod_basket_getAofY(xsect, y);

      elseif xsect.xsecttype == TRAPEZOIDAL return trapez_getAofY(xsect, y);

      elseif xsect.xsecttype == TRIANGULAR  return triang_getAofY(xsect, y);

      elseif xsect.xsecttype == PARABOLIC   return parab_getAofY(xsect, y);

      elseif xsect.xsecttype == POWERFUNC   return powerfunc_getAofY(xsect, y);
...
function xsect_getWofY(xsect::TXsect, y::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           y = depth ft)
...
#  Purpose: computes xsection's top width at a given depth.
    yNorm = y / xsect.yFull;
      ...
      elseif xsect.xsecttype == RECT_CLOSED
          if yNorm == 1.0; return 0.0 end
          return xsect.wMax;

      elseif xsect.xsecttype == RECT_TRIANG return rect_triang_getWofY(xsect, y);

      elseif xsect.xsecttype == RECT_ROUND  return rect_round_getWofY(xsect, y);

      elseif xsect.xsecttype == RECT_OPEN   return xsect.wMax;

      elseif xsect.xsecttype == MOD_BASKET  return mod_basket_getWofY(xsect, y);

      elseif xsect.xsecttype == TRAPEZOIDAL return trapez_getWofY(xsect, y);

      elseif xsect.xsecttype == TRIANGULAR  return triang_getWofY(xsect, y);

      elseif xsect.xsecttype == PARABOLIC   return parab_getWofY(xsect, y);

      elseif xsect.xsecttype == POWERFUNC   return powerfunc_getWofY(xsect, y);
...
function xsect_getRofY(xsect::TXsect, y::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           y = depth (ft)
...
#  Purpose: computes xsection's hydraulic radius at a given depth.
    yNorm = y / xsect.yFull;
      ...
      elseif xsect.xsecttype == RECT_TRIANG  return rect_triang_getRofY(xsect, y);

      elseif xsect.xsecttype == RECT_ROUND   return rect_round_getRofY(xsect, y);

      elseif xsect.xsecttype == TRAPEZOIDAL  return trapez_getRofY(xsect, y);

      elseif xsect.xsecttype == TRIANGULAR   return triang_getRofY(xsect, y);

      elseif xsect.xsecttype == PARABOLIC    return parab_getRofY(xsect, y);

      elseif xsect.xsecttype == POWERFUNC    return powerfunc_getRofY(xsect, y);

      else           return xsect_getRofA( xsect, xsect_getAofY(xsect, y) );
      end
end

################################################################################

function xsect_getRofA(xsect::TXsect, a::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           a = area (ft2)
#  Output:  returns hydraulic radius (ft)
#  Purpose: computes xsection's hydraulic radius at a given area.
    # double cathy;
    if a <= 0.0; return 0.0 end
      if xsect.xsecttype == HORIZ_ELLIPSE || xsect.xsecttype == VERT_ELLIPSE ||
            xsect.xsecttype == ARCH || xsect.xsecttype == IRREGULAR || 
            xsect.xsecttype == FILLED_CIRCULAR || xsect.xsecttype == CUSTOM ||
            xsect.xsecttype == STREET_XSECT
        return xsect_getRofY( xsect, xsect_getYofA(xsect, a) );

      elseif xsect.xsecttype == RECT_CLOSED  return rect_closed_getRofA(xsect, a);

      elseif xsect.xsecttype == RECT_OPEN    return a / (xsect.wMax +
                             (2. - xsect.sBot) * a / xsect.wMax);

      elseif xsect.xsecttype == RECT_TRIANG  return rect_triang_getRofA(xsect, a);

      elseif xsect.xsecttype == RECT_ROUND   return rect_round_getRofA(xsect, a);

      elseif xsect.xsecttype == MOD_BASKET   return mod_basket_getRofA(xsect, a);

      elseif xsect.xsecttype == TRAPEZOIDAL  return trapez_getRofA(xsect, a);

      elseif xsect.xsecttype == TRIANGULAR   return triang_getRofA(xsect, a);

      elseif xsect.xsecttype == PARABOLIC    return parab_getRofA(xsect, a);

      elseif xsect.xsecttype == POWERFUNC    return powerfunc_getRofA(xsect, a);

      else
        cathy = xsect_getSofA(xsect, a);
        if cathy < TINY || a < TINY; return 0.0 end
        return (cathy/a) ^ (3.0/2.);
      end
end

################################################################################

function xsect_getAofS(xsect::TXsect, s::Real)
#  Input:   xsect = ptr. to a cross section data structure
...
#  Purpose: computes xsection's area at a given section factor.
    ...
    if s > xsect.sMax; s = xsect.sMax end
      if xsect.xsecttype == DUMMY     return 0.0;

      elseif xsect.xsecttype == FORCE_MAIN || xsect.xsecttype == CIRCULAR  
      ...
      else return generic_getAofS(xsect, s);
      end
end

################################################################################

function xsect_getdSdA(xsect::TXsect, a::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           a = area (ft2)
...
function xsect_getYcrit(xsect::TXsect, q::Real)
#  Input:   xsect = ptr. to a cross section data structure
#           q = flow rate (cfs)
...
#  Purpose: computes critical depth at a specific flow rate.
    ...
    if q2g == 0.0; return 0.0 end
      if xsect.xsecttype == DUMMY
        return 0.0;

      elseif xsect.xsecttype == RECT_OPEN || xsect.xsecttype == RECT_CLOSED
        # --- analytical expression for yCritical is
        #     y = (q2g / w^2)^(1/3) where w = width
        y = (q2g / SQR(xsect.wMax)) ^ (1.0/3.);

      elseif xsect.xsecttype == TRIANGULAR
        # --- analytical expression for yCritical is
        #     y = (2 * q2g / s^2)^(1/5) where s = side slope
        y = (2.0 * q2g / SQR(xsect.sBot)) ^ (1.0/5.);

      elseif xsect.xsecttype == PARABOLIC
        # --- analytical expression for yCritical is
        #     y = (27/32 * q2g * c)^(1/4) where y = c*x^2
        #     is eqn. for parabola and 1/sqrt(c) = rBot
        y = (27.0/32. * q2g / SQR(xsect.rBot)) ^ (1.0/4.);
      ...
      elseif xsect.xsecttype == POWERFUNC
        ...
        y = 1.01 * (q2g / xsect.yFull) ^ (1.0/4.);
        if y >= xsect.yFull; y = 0.97 * xsect.yFull end
...
function lookup(x::Real, table::Vector{Float64}, nItems::Int)
...
#  Purpose: looks up a value in a geometry table (i.e., finds y given x).
    ...
    delta = 1.0 / (nItems-1);
    if (x / delta > 1e23)
      println("$(delta) = 1/($(nItems)-1)")
      println("x=$(x)")
    end
    i = floor(Int, x / delta) + 1 # EDIT: +1 to account for julia's 1-index
    if ( i >= nItems - 1 ) return table[nItems-1]; end
...
function invLookup(y::Real, table::Array{Float64}, nItems::Int)
...
#           The x value's in a geometry table lie between 0 and 1.
    ...
    if i >= n - 1; return ((double)n-1) * dx end
...
function locate(y::Real, table::Array{Real}, jLast::Int)
...
#  Notes:   This function is only used in conjunction with invLookup().
    ...
    if y <= table[0]; return 0 end

    # Check if value >= the last entry
    if y >= table[jLast]; return jLast end
...
################################################################################
# RECT_CLOSED functions
################################################################################

function rect_closed_getSofA(xsect::TXsect, a::Real)
    ...
    return a * xsect_getRofA(xsect, a) ^ (2.0/3.0)
end

function rect_closed_getdSdA(xsect::TXsect, a::Real)
    ...
    if alpha <= 1.0e-30; return generic_getdSdA(xsect, a) end

    # --- otherwise evaluate dSdA = [5/3 - (2/3)(dP/dA)R]R^(2/3)
    #     (where P = wetted perimeter & dPdA = 2/width)
    r = xsect_getRofA(xsect, a)
    return  (5.0/3. - (2.0/3.) * (2.0/xsect.wMax) * r) * r ^ (2.0/3.)
end

function rect_closed_getRofA(xsect::TXsect, a::Real)
    # double p;
    if a <= 0.0; return 0.0 end
...
function rect_open_getSofA(xsect::TXsect, a::Real)
    y = a / xsect.wMax
    r = a / ((2.0-xsect.sBot)*y + xsect.wMax)
    return a * r ^ (2.0/3.)
end


function rect_open_getdSdA(xsect::TXsect, a::Real)
    # double r, dPdA;

    # --- for small a/aFull use generic central difference formula
    if a / xsect.aFull <= 1.0e-30; return generic_getdSdA(xsect, a) end

    # --- otherwise evaluate dSdA = [5/3 - (2/3)(dP/dA)R]R^(2/3)
    #     (where P = wetted perimeter)
    r = xsect_getRofA(xsect, a);
    dPdA = (2.0 - xsect.sBot) / xsect.wMax; # since P = geom2 + 2a/geom2
    return  (5.0/3. - (2.0/3.) * dPdA * r) * r ^ (2.0/3.);
end

################################################################################
# RECT_TRIANG functions
################################################################################

function rect_triang_getYofA(xsect::TXsect, a::Real)
    # below upper section
...
function rect_triang_getRofA(xsect::TXsect, a::Real)
    # double y;
    # double p, alf;
    ...
    # below upper section
    if y <= xsect.yBot; return a / (2. * y * xsect.rBot) end
    ...
    if alf > 0.0; p += alf / (1.0 - RECT_TRIANG_ALFMAX) * xsect.wMax end
    return a / p
end

function rect_triang_getSofA(xsect::TXsect, a::Real)
    # --- if a > area corresponding to sMax, then
    #     interpolate between sMax and Sfull
    ...
    # --- otherwise use regular formula
    else return a * rect_triang_getRofA(xsect, a) ^ (2.0/3.);
    end
end

function rect_triang_getdSdA(xsect::TXsect, a::Real)
...
function rect_triang_getAofY(xsect::TXsect, y::Real)
...
function rect_triang_getRofY(xsect::TXsect, y::Real)
    # double p, a, alf;

    # y is below upper rectangular section
    if y <= xsect.yBot; return y * xsect.sBot / (2. * xsect.rBot) end
...
function rect_triang_getWofY(xsect::TXsect, y::Real)
...
################################################################################
# RECT_ROUND functions
################################################################################

function rect_round_getYofA(xsect::TXsect, a::Real)
    # double alpha;

    # --- if above circular bottom:
    if a > xsect.aBot; return xsect.yBot + (a - xsect.aBot) / xsect.wMax end

    # --- otherwise use circular xsection method to find height
    alpha = a / (PI * xsect.rBot * xsect.rBot);
    if alpha < 0.04; return (2.0 * xsect.rBot) * getYcircular(alpha) end
    return (2.0 * xsect.rBot) * lookup(alpha, Y_Circ, N_Y_Circ);
end

function rect_round_getRofA(xsect::TXsect, a::Real)
    ...
    if a > xsect.aBot
        ...
        if arg > 0.0; p += arg / (1.0 - RECT_ROUND_ALFMAX) * xsect.wMax end
        return a / p;
    end
...
function rect_round_getSofA(xsect::TXsect, a::Real)
    ...
    elseif a > xsect.aBot
        return a * xsect_getRofA(xsect, a) ^ (2.0/3.)
...
function rect_round_getdSdA(xsect::TXsect, a::Real)
...
function rect_round_getAofY(xsect::TXsect, y::Real)
    # double theta1;

    # --- if above circular invert...
    if y > xsect.yBot; return xsect.aBot + (y - xsect.yBot) * xsect.wMax end
...
function rect_round_getRofY(xsect::TXsect, y::Real)
    # double theta1;
...
function rect_round_getWofY(xsect::TXsect, y::Real)
    # --- return width if depth above circular bottom section
    if y > xsect.yBot; return xsect.wMax end
...
function mod_basket_getRofA(xsect::TXsect, a::Real)
    # double y1, p, theta1;
...
function mod_basket_getdSdA(xsect::TXsect, a::Real)
    # double r, dPdA;
...
function mod_basket_getAofY(xsect::TXsect, y::Real)
    # double a1, theta1, y1;

    # --- if water level is below top of rectangular bottom
    #     return depth * width
    if y <= xsect.yFull - xsect.yBot; return y * xsect.wMax end
...
function mod_basket_getWofY(xsect::TXsect, y::Real)
    # double y1;

    # --- if water level below top of rectangular bottom then return width
    if y <= 0.0; return 0.0 end
    if y <= xsect.yFull - xsect.yBot; return xsect.wMax end
...
function trapez_getYofA(xsect::TXsect, a::Real)
    if xsect.sBot == 0.0; return a / xsect.yBot end
    return ( sqrt( xsect.yBot*xsect.yBot + 4.0*xsect.sBot*a )
             - xsect.yBot )/(2. * xsect.sBot);
end

function trapez_getRofA(xsect::TXsect, a::Real)
    return a / (xsect.yBot + trapez_getYofA(xsect, a) * xsect.rBot)
end

function trapez_getdSdA(xsect::TXsect, a::Real)
    # double r, dPdA;
    # --- use generic central difference method for very small a
    if a/xsect.aFull <= 1.0e-30; return generic_getdSdA(xsect, a) end
...
function trapez_getAofY(xsect::TXsect, y::Real)
    return ( xsect.yBot + xsect.sBot * y ) * y
end

function trapez_getRofY(xsect::TXsect, y::Real)
    if y == 0.0; return 0.0 end
    return trapez_getAofY(xsect, y) / (xsect.yBot + y * xsect.rBot)
end

function trapez_getWofY(xsect::TXsect, y::Real)
...
function triang_getYofA(xsect::TXsect, a::Real)
    return sqrt(a / xsect.sBot);
end

function triang_getRofA(xsect::TXsect, a::Real)
    return a / (2. * triang_getYofA(xsect, a) * xsect.rBot);
end

function triang_getdSdA(xsect::TXsect, a::Real)
    # double r, dPdA;
...
function triang_getAofY(xsect::TXsect, y::Real)
    return y * y * xsect.sBot
end

function triang_getRofY(xsect::TXsect, y::Real)
    return (y * xsect.sBot) / (2. * xsect.rBot);
end

function triang_getWofY(xsect::TXsect, y::Real)
...
function parab_getYofA(xsect::TXsect, a::Real)
    return ((3.0/4.) * a / xsect.rBot) ^ (2.0/3.)
end

function parab_getRofA(xsect::TXsect, a::Real)
    if a <= 0.0; return 0.0 end
    return a / parab_getPofY( xsect, parab_getYofA(xsect, a) )
end

function parab_getPofY(xsect::TXsect, y::Real)
    x = 2. * sqrt(y) / xsect.rBot
    t = sqrt(1.0 + x * x)
    return 0.5 * xsect.rBot * xsect.rBot * ( x * t + log(x + t) )
end

function parab_getAofY(xsect::TXsect, y::Real)
    return (4.0/3. * xsect.rBot * y * sqrt(y))
end

function parab_getRofY(xsect::TXsect, y::Real)
    if y <= 0.0; return 0.0 end
    return parab_getAofY(xsect, y) / parab_getPofY(xsect, y)
end

    
function parab_getWofY(xsect::TXsect, y::Real)
...
function powerfunc_getYofA(xsect::TXsect, a::Real)
    return (a / xsect.rBot) ^ (1.0 / (xsect.sBot + 1.0))
end

function powerfunc_getRofA(xsect::TXsect, a::Real)
    if a <= 0.0; return 0.0 end
    return a / powerfunc_getPofY(xsect, powerfunc_getYofA(xsect, a))
end

function powerfunc_getPofY(xsect::TXsect, y::Real)
...
function powerfunc_getAofY(xsect::TXsect, y::Real)
    return xsect.rBot * y ^ (xsect.sBot + 1.0)
end

function powerfunc_getRofY(xsect::TXsect, y::Real)
    if y <= 0.0; return 0.0 end
    return powerfunc_getAofY(xsect, y) / powerfunc_getPofY(xsect, y)
end

function powerfunc_getWofY(xsect::TXsect, y::Real)
    return (xsect.sBot + 1.0) * xsect.rBot * y ^ xsect.sBot
end
...
function circ_getYofA(xsect::TXsect, a::Real)
    alpha = a / xsect.aFull
...
function circ_getAofS(xsect::TXsect, s::Real)
    psi = s / xsect.sFull
    if psi == 0.0; return 0.0 end
    if psi >= 1.0; return xsect.aFull end
...
function circ_getSofA(xsect::TXsect, a::Real)
    alpha = a / xsect.aFull
    ...
    # --- otherwise use table
    else return xsect.sFull * lookup(alpha, S_Circ, N_S_Circ)
    end
end

function circ_getdSdA(xsect::TXsect, a::Real)
...
function circ_getAofY(xsect::TXsect, y::Real)
    # double yNorm;
    yNorm = y / xsect.yFull
    return xsect.aFull * lookup(yNorm, A_Circ, N_A_Circ)
end
...
function filled_circ_getYofA(xsect::TXsect, a::Real)
    # double y;
...
function filled_circ_getAofY(xsect::TXsect, y::Real)
    # double a;
...
function filled_circ_getRofY(xsect::TXsect, y::Real)
    # double a, r, p;
...
function getYcircular(alpha::Real)
    # double theta;
    if alpha >= 1.0; return 1.0 end
    if alpha <= 0.0; return 0.0 end
    if alpha <= 1.0e-5
        theta = (37.6911*alpha) ^ (1.0/3.)
...
function getScircular(alpha::Real)
    # double theta;
    if alpha >= 1.0; return 1.0 end
    if alpha <= 0.0; return 0.0 end
    if alpha <= 1.0e-5 
        theta = (37.6911*alpha) ^ (1.0/3.);
        return theta ^ (13.0/3.) / 124.4797;
    end
    theta = getThetaOfAlpha(alpha);
    return (theta - sin(theta)) ^ (5.0/3.) / (2.0 * PI) / theta ^ (2.0/3.)
end

function getAcircular(psi::Real)
    # double theta;
    if psi >= 1.0; return 1.0 end
    if psi <= 0.0; return 0.0 end
    if psi <= 1.0e-6
        theta = (124.4797*psi)^ (3.0/13.)
...
function getThetaOfAlpha(alpha::Real)
    # int    k;
    ...
    for k = 1:40
        ...
        if d > 1.0; d = SIGN( 1.0, d ) end
        theta = theta - d
        if fabs(d) <= 0.0001; return theta end
    end
    return theta1
end

function getThetaOfPsi(psi::Real)
    # int    k;
    # double theta, theta1, ap, tt, tt23, t3, d;

    if     psi > 0.90;  theta = 4.17 + 1.12 * (psi - 0.90) / 0.176
    elseif psi > 0.5;   theta = 3.14 + 1.03 * (psi - 0.5) / 0.4;
    elseif psi > 0.015; theta = 1.2 + 1.94 * (psi - 0.015) / 0.485
    else                theta = 0.12103 - 55.5075 * psi + 15.62254 * sqrt(psi)
    end
    ...
    for k = 1:40
        ...
        d        = d / ( ap*(2.0/3.)/t3 - (5.0/3.)*tt23*(1.0-cos(theta)) )
        theta    = theta - d
        if fabs(d) <= 0.0001; return theta end
    end
    return theta1
